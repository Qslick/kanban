...
describe("board dependency state", () => {
	...
	it("preserves task-level cline overrides when updating the title", () => {
		...
		expect(updatedTask?.agentSettings).toEqual({
			providerId: "openrouter",
			modelId: "openai/gpt-5.4",
			reasoningEffort: "low",
		});
	...
	it("preserves model fields when disabling auto-review", () => {
		...
		expect(task.agentSettings).toEqual({
			providerId: "my-provider",
			modelId: "my-model",
			reasoningEffort: "high",
		});
	...
	it("does not create task model overrides for tasks inheriting global agent settings", () => {
		...
		const result = applyTaskDetailAgentSettingsSelection(board, task.id, {
			agentId: "cline",
			agentSettings: {
				providerId: "openrouter",
		...
		expect(unchangedTask?.agentSettings).toBeUndefined();
	});
