...
- Panel review is fail-closed auto-review: inherit with no seats after excluding the implementer family skips and allows auto-review; custom with zero successful seats parks (`rejected`). Seats are read-only — never `--always-approve`/`--yolo` for a Grok reviewer. Types live in `src/core/panel-review.ts`; `dispatchPanelSeats` is injected so tests never call real CLIs.
