...
- In `web-ui`, prefer `react-use` hooks (via `@/utils/react-use`) whenever possible
...
- Trashing a card when its session goes `interrupted` (`use-board-interactions.ts`) is edge-triggered and must stay that way. `previousSessionsRef` starts empty, so `previous` is undefined the first time any session is seen — on every mount and project switch. A guard of `previous?.state !== "interrupted"` reads a session that was *already* interrupted before load as a fresh transition and silently moves that card to Done just by opening the board. Require `previous` to exist. Note the ref is deliberately cleared inside that effect on a project change, not from `resetBoardInteractionsState`: effects run in declaration order and the later reset would clobber the seed the reconciliation effect just wrote. Stranded sessions are recovered through the card's Keep/Resume/Discard flow.
- `sessions.json` grows without bound and nothing prunes it. Two causes, one fixed: `use-workspace-sync.ts` used to *merge* session records across a project switch, so every board accumulated the other boards' task ids and `__home_agent__:<other-workspace>:*` keys (marketply reached 341 records / 971 KB for 12 cards; job-hunter 305 of 311 orphaned). It now replaces on `!isSameProject`. Still unfixed: deleting a card leaves its session record forever, and a stale browser tab that reconnects without a reload re-merges its old records into a cleaned file. The durable fix is a server-side prune on load, but it must not key purely on board membership — a transient empty board during hydration would wipe real session history.
- The per-workspace `TerminalSessionManager` hydrates from the whole persisted sessions record (`workspace-registry.ts` `ensureTerminalManagerForWorkspace`) and `saveState` writes every summary it lists back (`workspace-api.ts`). That is a self-sustaining loop: cleaning `sessions.json` only sticks if the runtime is restarted afterwards, and nothing shrinks the file on its own.
- Never hand-edit anything under `~/.cline/kanban/workspaces/` with a plain truncate-then-write while a runtime is live. The app writes through `writeJsonFileAtomic` under a lock; a non-atomic rewrite of `index.json` made a live runtime read it mid-write and log `Workspace <id> is no longer registered`. Use a sibling temp file plus `os.replace`/rename, and prefer stopping the runtime first.
- `react-use`'s `useLocalStorage` memoizes its setter on `[key, setState]` but reads `state` from the closure, so the functional-updater form always sees the value from the render where the key last changed. A raw `setValue((current) => !current)` toggle therefore works exactly once per mount and then sticks — this is what made Ready-now-hidden cards unrecoverable. Always go through `useBooleanLocalStorageValue` / `useRawLocalStorageValue` in `web-ui/src/utils/react-use.ts`: they resolve updates against a ref and hand `react-use` a plain value, never a function. Regression cover is `web-ui/src/utils/react-use.test.tsx`.
- The Backlog `Ready now` toggle is the only thing in the UI that removes a card from the rendered board, and the set it hides is exactly "cards with a dependency link". It is render-only, so a hidden card is still in `board.json` — which reads as data loss. Anything that hides cards must keep the true total in the count badge and offer a way back (`board-column.tsx`), and the card matching `editingTaskId` must stay visible because the inline editor is rendered from the filtered list. The filter key is scoped per workspace path; a global key hid blocked cards on every project at once.
- `updateTaskDependencies` runs on every board read on both the server (`readWorkspaceBoard`) and the client (`normalizeBoardData`), and its result is what gets persisted. Anything it drops is gone for good and any reorientation silently rewrites the user's link. It must only retire a link whose endpoint left the board or reached Done, and must never touch direction. Column rules that decide whether a link may be *created* belong in `resolveDependencyEndpoints`, which is creation-only — it previously ran during normalization too, which deleted every link once both endpoints left Backlog and inverted links when the waiting card started.
- Dependency links are identified by the *unordered* pair of tasks, so `B -> A` is a `duplicate` of `A -> B`; `addTaskDependency`/`canAddTaskDependency` also refuse a link that would close a longer loop (`reason: "cycle"`). A loop leaves every card in it permanently blocked and impossible to auto-start.
- The dependency overlay paints at `z-index: 4` with 14px-wide `pointer-events: stroke` hit paths, above the cards' `z-index: 1`. Any new interactive control placed inside `.kb-column-cards` needs `relative z-10` or the dependency edges silently eat its clicks.
- `test/runtime/git-history.test.ts > reads ahead and behind counts from tracked branches` is intermittently flaky and unrelated to whatever you changed. Re-run before investigating.
...
- Machine-default model/effort is probed from each provider's computer-wide config (`src/core/agent-machine-defaults.ts`). Unset card `modelId`/`reasoningEffort` means inherit the current machine default — do not copy the default onto new cards. Launch omits flags when inheriting (except the existing Cline SDK path). Codex: never `-m` unless the card pinned a model (ChatGPT-account 400).
